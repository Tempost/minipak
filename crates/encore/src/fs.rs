use crate::{
    error::EncoreError,
    memmap::{FileOptions, MmapOptions},
    syscall::{self, FileDescriptor, MmapProtection, OpenFlags, Stat},
};
...
impl File {
    ...
    pub fn open(path: &str) -> Result<Self, EncoreError> {
        Self::raw_open(path, OpenFlags::READONLY, 0)
    }
    ...
    pub fn map(&self) -> Result<Map<'_>, EncoreError> {
        let self_data = MmapOptions::new(self.len()?)
            ...
            .protection(MmapProtection::READ)
            .map()? as *const u8;
